<template>
     <div class="h-full w-full rounded-md bg-[#1f1d2b] text-white p-8">
      <!-- Header -->
      ...
      <div class="grid grid-cols-3 gap-6 mb-8">
        ...
        <div
          @click="openAddModal"
          class="border-2 border-dashed border-gray-600 rounded-xl flex flex-col items-center justify-center h-[340px] cursor-pointer hover:border-[#ff6b6b] transition-colors group"
        >
          <div
        ...
        <div
          ...
          :key="dish.id"
          class="bg-[#2d303e] rounded-xl overflow-hidden"
        >
          <div class="p-6">
            <div
  ...
  <button
            ...
            class="w-full bg-[#50343A] py-4 flex items-center justify-center gap-2 text-sm border-r-2"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
          ...
          <button
            ...
            class="w-full bg-[#50343A] py-4 flex items-center justify-center gap-2 text-sm"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
...
export default {
  ...
  computed: {
    ...
    hasChanges() {
      return JSON.stringify(this.dishes) !== JSON.stringify(this.workingDishes);
    },
  },
  methods: {
    loadDishes() {
      const saved = localStorage.getItem("restaurant_dishes");
      this.dishes = saved ? JSON.parse(saved) : this.initialDishes;
      this.workingDishes = [...this.dishes];
    },
    saveChanges() {
      this.dishes = [...this.workingDishes];
    ...
    deleteDish(dishId){
     ...
     this.workingDishes = this.workingDishes.filter(d => d.id !== dishId);
      }
    },
    ...
    submitForm() {
      ...
      if (this.isEditing) {
        ...
        if (index !== -1) {
          this.workingDishes[index] = { id: this.editingId, ...this.formData };
        }
      } else {
        const newDish = {
          id: Date.now(),
